...
def Input():
    psspy.prompt("INPUT:           \n\
    - SCALE : BAC / TRUNG / NAM    \n\
    - 10: BAC : NPC + HN + KH_BAC  \n\
    - 30: TRUNG : CPC + KH_TRUNG   \n\
    - 20: NAM : SPC + HCM + KH_NAM \n")
    ierr, i_in = psspy.userin() # Nhap area scale
    psspy.prompt(" Nhap Scale Value: \n")   # Nhap value of scale
    ierr, i_scale = psspy.userin()   
    comp = [10,20,30]
    check = 0
    for i in comp :    
        if i == int(i_in):
            check = 1
    return check,int(i_in),float(i_scale)
def gen_sub(Ax):
    if Ax == 10:
...
def sum_matrix (M) :
    ...
    for i in range(len(M[0])):
        Sum = Sum + M[0][i]
    return Sum
i_ch , i_Ax , i_scale = Input() # Nhap AREA / SCALE VALUE
psspy.prompt("SCALE Q METHOD  \n\
    O : No change Q           \n\
    1 : Constant P/Q ratio    \n\
    Issue : 15/01/2021        \n") 
ierr, scale_mth = psspy.userin() # NHAP SCALE _ METHOD
